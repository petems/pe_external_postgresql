...
# table_creator.
define pe_external_postgresql::default_write_grant(
  String $database,
  String $schema,
  String $table_creator,
  String $table_writer,
  ) {
    pe_external_postgresql::psql {"${title}/tables":
      db      => $database,
    ...
    pe_external_postgresql::psql {"${title}/sequences":
      db      => $database,
      command => "ALTER DEFAULT PRIVILEGES
                  FOR USER \"${table_creator}\"
                  IN SCHEMA \"${schema}\"
    ...
    pe_external_postgresql::psql {"${title}/functions":
      db      => $database,
      command => "ALTER DEFAULT PRIVILEGES
                  FOR USER \"${table_creator}\"
                  IN SCHEMA \"${schema}\"
